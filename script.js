    ...
    try {
        ...
        manuscripts.forEach((manuscript, index) => {
            ...
            switch (manuscript.group) {
                case 'Γ':
                    titleColor = 'red'; // Colour for group Γ
                    break;
                case 'Λ':
            ...
            manuscriptsList.innerHTML += `
                <div class="manuscript" 
                    ...
                    data-composite="${manuscript.composite === 'yes' ? 'composite' : ''}"
    ...
    .then(data => {
        ...
        data.forEach(manuscript => {
            if (manuscript.imageUrl) { // Ensure the imageUrl exists
                ...
                galleryItem.setAttribute('data-composite', manuscript.composite === 'yes' ? 'composite' : '');
        ...
        function filterGallery() {
            ...
            galleryItems.forEach(galleryItem => {
                ...
                const matchesComposite = compositeFilter === '' || composite === 'composite';
...
async function fetchAllTexts() {
  ...
  // Load table of manuscript shelfmarks and sigla
async function loadSiglaTable() {
    try {
        ...
        const manuscripts = await response.json();
        const tableBody = document.getElementById('sigla-table-body');

        tableBody.innerHTML = '';

       // Group manuscripts by family
const grouped = {};

manuscripts
    .filter(manuscript => manuscript.siglum && manuscript.siglum.trim() !== '')
    .forEach(manuscript => {
        const group = manuscript.group || 'Unknown';

        if (!grouped[group]) {
            grouped[group] = [];
        }

        grouped[group].push(manuscript);
    });

const groupOrder = ['Γ', 'Λ', 'Unknown'];

groupOrder.forEach(group => {
    if (!grouped[group]) return;

    grouped[group].sort((a, b) => a.siglum.localeCompare(b.siglum));
    ...
    tableBody.innerHTML += `
        <tr class="group-header">
            <td colspan="2" style="color: ${titleColor}; font-weight: bold;">
                 ${group}
            </td>
        </tr>
    `;

    grouped[group].forEach(manuscript => {
        const manuscriptIndex = manuscripts.indexOf(manuscript);

        tableBody.innerHTML += `
            <tr>
                <td>
                    <a target="_blank" 
                       href="manuscript.html?id=${manuscriptIndex}"
                       style="color: ${titleColor}; text-decoration: none;">
        ...
        console.error('Error loading sigla table:', error);
        document.getElementById('sigla-table-body').innerHTML = `
            <tr>
                <td colspan="2">Error loading sigla table.</td>
            </tr>
        `;
    }
}